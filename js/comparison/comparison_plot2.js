...
const id_ref_2 = "#boxplot"

// Set the dimensions and margins of the graph
const margin_2 = {top: 50, right: 20, bottom: 70, left: 70},
    width_2 = 512 - margin_2.left - margin_2.right,
    height_2 = 384 - margin_2.top - margin_2.bottom;
...
// Y axis
var y_2 = [];
var y_label_2 = ["Life expectancy ","Adult mortality","BMI","Income composition of resources","Schooling"];
...
function draw2() {
    ...
    //  .rollup(function(d) {
    var sumstat = Array.from(d3.group(data, d => d.Status))
        .map(function(d) {
    ...
    {
        current_Status = sumstat[i].key
        current_max = sumstat[i].max
        current_min = sumstat[i].min

        for(j = 0; j < data.length; ++j)
        {
            if ((data[j].Status === current_Status) && ((data[j][measureHeading_2] < current_min) || (data[j][measureHeading_2] > current_max)))
                {
                    outlier.push(data[j])
                }
        }
    ...
    svg_2.selectAll("rect")
        ...
        .on("mouseover", function (event, d) {
            ...
            tooltip.html("<span class='tooltiptext'>" + "<b>Status: " + d.key + "</b>" + 
